...
/** Represents the global search space of all states S. It must include an initial state and a goal state. The type
  * parameters S and T correspond to a state and a transition from one state to the next. S and T must be immutable and
  * should calculate and cache distance metrics in the constructor if possible.
  *
  * @author
  *   Barry Becker
  */
trait SearchSpace[S, T] extends Refreshable[S, T] {
  ...
  /** Add the state to the "seen" set of states, if not already seen.
    ...
    *   Map of seen states.
    * @return
    *   true if the specified state was already seen (possibly taking into account symmetries).
    */
  ...
  def getCost(state: S, transition: T): Int
