...
import com.barrybecker4.search.space.TrivialSearchSpace
...
/** Sequential search strategy that uses the A* search algorithm. See http://en.wikipedia.org/wiki/A*_search_algorithm S
  * represents a state in the global search space. T represents a transition from one state to the next.
  *
  * The performance of this search is very dependent on the design of the search space. Here are some possible
  * optimizations to consider when designing the SearchSpace and its components.
  ...
  *   - When creating neighbors, use the fact that there is going to be an incremental change to the distance and do not
  *     recompute it from scratch. Hint: use a private constructor, that takes the distance as a param.
  *   - Sort the neighbors so that the most promising is delivered first.
  *
  * @param searchSpace
  *   the global search space containing initial and goal states.
  * @param openQueue
  *   the specific updatable priority queue to use. Candidate nodes to search on the frontier.
  * @author
  *   Barry Becker
  */
class AStarSearch[S, T](
    val searchSpace: SearchSpace[S, T],
  ...
  /** Best first search for a solution.
    * @return
    *   the solution state node, if found, which has the path leading to a solution. Null if no solution.
    */
